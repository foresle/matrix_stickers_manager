...
class MatrixStickersManager:
    ...
    def _check_max_media_upload_size(self) -> int:
        response = requests.get(
            f"https://{self._config.matrix_domain}/_matrix/media/v3/config"
    ...
    def _upload_media(self, file_path: str, image_type_only: bool = True) -> str:
        ...
        if os.path.getsize(file_path) > self._config.max_media_upload_size:
            raise MatrixStickersManagerError(
                text=f"File is so large. "
        ...
        with open(file_path, "rb") as file:
            response = requests.post(
                f"https://{self._config.matrix_domain}/_matrix/media/v3/upload"
                f"?filename={filename}"
                f"&access_token={self._config.matrix_token}",
    ...
    def _get_room_state(self, pack_name: str, room_id: str) -> dict:
        ...
        response = requests.get(
            f"https://{self._config.matrix_domain}/_matrix/client/v3/rooms/{room_id}"
    ...
    def _make_pack_obj(
        self, name: str, room_id: str | None = None, new_if_missing: bool = True
    ) -> dict:
        """
        ...
        pack: dict = {"pack": {"display_name": name}, "images": {}}
    ...
    @staticmethod
    def _add_sticker_to_pack(
        pack: dict, shortcode: str, image_mxc: str, usage: str | None = None
    ):
        """
        Add sticker to pack.
        ...
        if pack["images"].get(shortcode, False):
            raise MatrixStickersManagerError(
                f'This shortcode "{shortcode}" already exist.'
            )

        if usage is not None:
            pack["images"][shortcode] = {"url": image_mxc, "usage": (usage,)}
        else:
            pack["images"][shortcode] = {"url": image_mxc}
    ...
    def load_pack_from_folder(
        self,
        pack_name: str,
        folder_path: str,
        room_id: str,
        usage: str | None = None,
        number_as_shortcode: bool = False,
        skip_duplicate_errors: bool = False,
        skip_upload_errors: bool = False,
        ...
        for file_path in all_files:
            # Upload image
            try:
                image_url: str = self._upload_media(
    ...
    def _is_server_admin(self, user_id: str | None = None) -> bool:
        ...
        if user_id is None:
            response = requests.get(
                f"https://{self._config.matrix_domain}/_matrix/client/v3/account/whoami"
                f"?access_token={self._config.matrix_token}"
            )
        ...
        response = requests.get(
            f"https://{self._config.matrix_domain}/_synapse/admin/v1/users/{user_id}/admin",
    ...
    def _protect_media(self, mxc_url: str) -> None:
        ...
        response = requests.post(
            f"https://{self._config.matrix_domain}"
    ...
    def export_pack(
        ...
        pack = self._make_pack_obj(
            name=pack_name, room_id=room_id, new_if_missing=False
        )
        ...
        for count, image in enumerate(pack["images"].values(), start=1):
            image_url = self._assemble_mxc_url(image["url"])
            response = requests.get(
                f"https://{self._config.matrix_domain}/_matrix/media/v3/download"
